import maxlength from './maxlength';

describe('constraints/maxlength', () => {
  describe('validation', () => {
    it.each([
      ['2', 'foo'],
      ['3', 'foo-bar'],
    ])('should throw when max length does not match', (maxlLength, value) => {
      expect(() => {
        maxlength('page', value, maxlLength);
      }).toThrow();
    });

    it.each([
      ['3', 'fo'],
      ['3', 'foo'],
      ['4', 'foo'],
      ['10', 'foo-bar'],
    ])('should not throw when max length are in range', (maxlLength, value) => {
      expect(() => {
        maxlength('page', value, maxlLength);
      }).not.toThrow();
    });
  });

  describe('verify()', () => {
    it('should throw when no constraint value is provided', () => {
      expect(() => {
        maxlength.verify('page', '');
      }).toThrow(/requires a max length value/);
    });

    it('should throw when non-number or decimal constraint value is provided', () => {
      expect(() => {
        maxlength.verify('page', 'a');
      }).toThrow(/requires an integer value/);
      expect(() => {
        maxlength.verify('page', '1.1');
      }).toThrow(/requires an integer value/);
    });

    it('should throw when zero or negative number constraint value is provided', () => {
      expect(() => {
        maxlength.verify('page', '0');
      }).toThrow(/requires a value greather or equal to 1/);
      expect(() => {
        maxlength.verify('page', '-1');
      }).toThrow(/requires a value greather or equal to 1/);
    });
  });

  describe('toRegExp()', () => {
    it('should return the decimal RegExp source', () => {
      expect(maxlength.toRegExp('')).toEqual('.*');
    });

    const regexp = new RegExp(`^(?:${maxlength.toRegExp('')})(?![\\s\\S])`, 'su');

    const values = [
      // numbers
      '-1',
      '-1.5',
      '0',
      '1',
      '99',
      '0.1',
      '1.0',
      '12.34',

      // latin / accents
      'foo',
      'foo-bar',
      'fußball',
      'águia',
      'ação',
      'crème brûlée',
      'niño',
      'São Paulo',
      'München',

      // non-latin scripts
      '你好',
      'こんにちは',
      '안녕하세요',
      'Привет',
      'مرحبا',
      'שלום',
      'हिन्दी',
      'বাংলা',
      'ไทย',

      // symbols / punctuation / spacing
      '!@#$%^&*()',
      '[]{}<>',
      'foo_bar',
      'foo.bar',
      'foo/bar',
      'foo\\bar',
      'foo:bar',
      'foo;bar',
      'foo,bar',
      'foo bar',
      ' leading',
      'trailing ',
      'multiple   spaces',

      // unicode edge cases
      '👋',
      '🚀🔥',
      'café',
      'cafe\u0301', // same visual result as café, but using combining mark
      '中文-English-123',
      'العربية-123',
      '👨‍👩‍👧‍👦', // zero-width-joiner sequence

      // line breaks / tabs
      'foo\nbar',
      'foo\r\nbar',
      'foo\tbar',
    ];

    it.each(values)('should match any value "%s"', (value) => {
      expect(regexp.test(value)).toBe(true);
    });

    // it.each([[''], ['foo'], ['1.'], ['.1'], ['1e5'], ['1.2.3']])(
    //   'should not match invalid decimal value "%s"',
    //   (value) => {
    //     const regexp = new RegExp(`^${maxlength.toRegExp('')}$`);

    //     expect(regexp.test(value)).toBe(false);
    //   },
    // );
  });
});
